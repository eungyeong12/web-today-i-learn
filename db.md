...
중복되는 컬럼은 `nickname`이다.  
한 크루의 출석 기록이 여러 날짜에 반복 저장되면서 `crew_id - nickname` 매핑 정보가 계속 중복된다.

### 2. `attendance` 테이블에서 중복을 제거하기 위해 `crew` 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?

크루의 고유 정보만 따로 관리하는 테이블로 분리한다.

- `crew_id`: 크루 식별자
- `nickname`: 크루 닉네임

### 3. `crew` 테이블에 들어가야 할 크루들의 정보는 어떻게 추출할까? (hint: DISTINCT)
`DISTINCT`를 사용해 중복 없이 추출한다.
```roomsql
...
nickname 컬럼이다.
이제 닉네임은 crew 테이블에서 관리하므로 attendance에는 필요 없다.
